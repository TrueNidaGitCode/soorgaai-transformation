...
| How the records get in, and stay in step | [docs/sources-in-the-application.md](docs/sources-in-the-application.md) | The industry names its sources (a folder of sheets and WhatsApp for an academy; Jira and Confluence for a software team) in its overlay; Eame ships only the connectors those call for; every landing follows one keyed rule so the sheet and the chat produce one record, never two. |
