...
**Orphaned advisor feature**: `backend/trunida-backend/services/advisorService.js` + `controllers/advisorController.js` (route: `POST /strategy-canvas/advisor/ask`) are fully built and still respond if called directly, but the frontend module that would call them (`frontend/domain/advisor.js`) is never loaded by any HTML page — confirmed no `<script src>` reference anywhere in the frontend. A real user cannot reach this feature through the site. The chat panel actually live in the workspace today calls a *different* service, `blueprintSuggestService.js` (`POST /strategy-canvas/blueprint-suggest`), which duplicates the same "advisor" concept with its own separate system prompt and retrieval logic. Don't confuse the two, and don't assume `advisorService.js` is live just because it exists and works.
