...
export function buildPrompt(spec) {
  ...
  const system = [
    ...
    '    read THAT file, delete the rows of that dataset whose _source is "sample", insert',
    '    the rows from the file, and return { message: "<n> rows" }. Rows whose _source is',
    '    "own" belong to the owner and are NEVER deleted or replaced by sample rows, on any',
    '    call. Match datasetName against the dataset names in the DATA list above, exactly;',
    ...
    '    plus _source="own"; read it with the same parser.',
