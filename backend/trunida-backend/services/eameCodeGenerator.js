...
export function buildPrompt(spec) {
  ...
  const system = [
    ...
    '',
    'WHAT YOU MUST WRITE. All four, every time. A project missing any of them is',
    'rejected before anything else is checked, because it is not an application:',
    '  models/       the data. ONE collection unless the use case truly needs more —',
    '                a model per CSV file is not a design, it is a transcription, and',
    '                a generation that stops after writing them has delivered nothing.',
    '  services/     the logic that answers the question. This is the product.',
    '  routes/       an express Router, or the application exposes nothing to call.',
    '  frontend/app.js  drives the page, or there is nothing to open.',
    'A controller and a seed script alongside them. Write the routes and the UI',
    'FIRST if you are unsure of your budget: a project with logic and no way to',
    'reach it is worth less than a plain one that works.',
    '',
    ...
    '                                 — the conduct in services/assistant.js is applied to',
    '                                   every call; your systemPrompt is the subject only',
    '                                 -> resolves to { text, ... }',
    ...
    '  - scripts/seed<Something>.js must `export default async function seed({ datasetName, filePath } = {})`',
    '  - CALLED WITH NO ARGUMENTS on boot: it must be SAFE TO CALL EVERY TIME -- count each',
    '    collection first and return early if it already holds rows. Returning { message }',
    '    logs a useful line.',
    '  - CALLED WITH { datasetName, filePath } by the Data page (a fixed part of the runtime,',
    '    controllers/dataController.js) when the owner imports their own file for ONE dataset:',
    '    read THAT file, delete the rows of that dataset whose _source is "sample" AND the',
    '    rows whose _source equals the value the file rows carry (every row in one file has',
    '    the same _source: "own", "whatsapp", "jira", "confluence" or "github"), then insert',
    '    the rows from the file, and return { message: "<n> rows" }. So a second import or a',
    '    repeated sync replaces its own earlier rows and never duplicates them. Rows with any',
    '    other non-sample _source belong to the owner and are NEVER deleted or replaced by',
    '    sample rows, on any call. Match datasetName against the dataset names in the DATA list above, exactly;',
    '    if it names a dataset this script does not seed, return { message: "not mine" }',
    '    and touch nothing.',
    '  - the file the owner imports has the SAME COLUMNS as the sample file for that dataset',
    '    plus a _source column; read it with the same parser.',
    ...
    'in the DOM when your module runs. The page opens on a front door (sign in) and a',
    'welcome; its own script moves the visitor through those to the chat, and your module',
    'never touches them -- the chat may be hidden when you run, which changes nothing',
    'about what you do. Drive these elements:',
    ...
    'syntax.',
    '',
    'HOW THE APPLICATION SPEAKS IS NOT YOURS TO DECIDE. services/assistant.js holds the',
    'conduct — answer first, evidence under it, say plainly what cannot be told from the',
    'connected data, invent nothing, no Markdown, never name Svarg\'s stages — and',
    'services/llmService.js applies it to every generate() call. It is the same in every',
    'Svarg application and you do not restate it, weaken it or override it.',
    '',
    'So your system prompt carries ONLY WHAT YOU KNOW THAT IT CANNOT: what this',
    'application is about, what its records mean, which columns matter, how to read them.',
    'Do NOT write a persona line. Do NOT name yourself Eame, or an assistant, or a bot —',
    'the application answers as itself and the conduct says so. Do NOT repeat format',
    'rules. A prompt that opens "You are Eame, the assistant for..." is wrong twice: it',
    'names Svarg\'s machinery to the customer, and it duplicates what is already there.',
    '',
    ...
    '',
    'THE PROSE AND THE CARDS ARE ONE LIST, COUNTED ONCE. Build the list of records',
    'first, then tell the model how many it has and hand it exactly those records; the',
    'cards are that same list. A prose line saying "4 players missed practice" above six',
    'cards is the customer catching the application contradicting itself in one screen,',
    'and it happens whenever the sentence is written from one query and the cards from',
    'another. Count in code, not in the model.',
    '',
    'ONE ROW PER THING THE SENTENCE COUNTS. If the sentence counts PLAYERS, the list is',
    'players — a player who appears in two batches is one card, not two. Deduplicate on',
    'whatever the sentence is about before you count it or render it.',
    '',
  ...
  const user = [
    ...
    'A SIMPLE application. The runtime already provides the front door, the welcome, the',
    'Data page, feedback and the connectors; you write only the chat that answers. One',
    'model per dataset, one service, one controller, one route file, one seed script and',
    'one app.js. No dashboards, no admin pages, no settings, no extra screens, no styling.',
    '',
