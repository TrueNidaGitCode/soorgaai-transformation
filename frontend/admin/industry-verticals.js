 ...
 * additional grounding context for any company tagged with that vertical
 * (see getVerticalContextForCapability in
 * backend/trunida-backend/services/industryVerticalKnowledgeService.js).
 *
 * Structurally a near-copy of company-library.js — same review/approve/
 * discard/edit component shapes, same eb-sidebar/eb-panel/eb-draft CSS
 * classes, adapted to (parentIndustry, subVertical) entities instead of
 * companies.
 *
 * Client-side role guard only — the backend independently enforces
 * adminOnly on every /api/admin/industry-verticals/* route.
 */
...
async function api(path, opts = {}) {
  const res = await fetch(`${API_BASE}/admin/industry-verticals${path}`, { ...opts, headers: { ...authHeaders(), ...(opts.headers || {}) } });
...
function hideBanner() {
  document.getElementById('iv-banner').style.display = 'none';
}

// ── State ──────────────────────────────────────────────────────────────────

let currentEntry = null;   // full vertical entry when in detail view
let selected     = null;   // { capabilityId, sectionTitle }

// ── List view ──────────────────────────────────────────────────────────────

async function loadList() {
  document.getElementById('iv-list-loading').style.display = 'flex';
  document.getElementById('iv-table').style.display = 'none';
  document.getElementById('iv-list-empty').style.display = 'none';

  try {
    const { entries } = await api('/');
    document.getElementById('iv-list-loading').style.display = 'none';

    if (!entries.length) {
      document.getElementById('iv-list-empty').style.display = 'block';
      return;
    }
    ...
    tbody.innerHTML = entries.map(e => `
      <tr class="cl-table__row" data-id="${esc(e._id)}">
        <td>${esc(e.parentIndustry)}</td>
        <td>${esc(e.subVertical)}</td>
    ...
    document.getElementById('iv-table').style.display = 'table';
  } catch (err) {
    document.getElementById('iv-list-loading').style.display = 'none';
    showBanner(err.message || 'Failed to load verticals.');
  }
}
...
async function handleCreate(evt) {
  evt.preventDefault();
  hideBanner();
  const parentIndustry = document.getElementById('iv-parent-industry').value;
  const subVertical     = document.getElementById('iv-sub-vertical').value.trim();
  if (!subVertical) return;

  const btn = document.getElementById('iv-create-btn');
  btn.disabled = true;
  try {
    const { entry } = await api('/', { method: 'POST', body: JSON.stringify({ parentIndustry, subVertical }) });
    document.getElementById('iv-sub-vertical').value = '';
    await loadList();
    openDetail(entry._id);
  } catch (err) {
    showBanner(err.message || 'Failed to create vertical.');
  } finally {
    btn.disabled = false;
  }
}
...
function showListView() {
  history.replaceState(null, '', 'industry-verticals.html');
  document.getElementById('iv-detail-view').style.display = 'none';
  document.getElementById('iv-list-view').style.display = 'block';
  currentEntry = null;
  selected = null;
  loadList();
}
...
async function openDetail(id) {
  hideBanner();
  history.replaceState(null, '', `industry-verticals.html?id=${encodeURIComponent(id)}`);
  document.getElementById('iv-list-view').style.display = 'none';
  document.getElementById('iv-detail-view').style.display = 'block';
  selected = null;
  ...
  try {
    ...
    showBanner(err.message || 'Failed to load vertical.');
  }
}

function renderDetailHeader() {
  document.getElementById('iv-detail-title').textContent = currentEntry.subVertical;
  document.getElementById('iv-detail-meta').textContent = `${currentEntry.parentIndustry} · Status: ${currentEntry.status}`;
}
...
function renderSidebar() {
  const nav = document.getElementById('iv-sidebar');
  nav.innerHTML = '';
...
function buildDraftReview(section) {
  ...
  flag.textContent = section.draftSource === 'external-research'
    ...
    : 'Limited public information found — drafted at a general level. Review carefully.';
...
async function runResearch() {
  hideBanner();
  const btn = document.getElementById('iv-research-btn');
  btn.disabled = true;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (!token || role !== 'admin') {
    localStorage.setItem('redirectAfterLogin', '/admin/industry-verticals.html');
    window.location.href = '/admin/login.html';
    return;
  }

  document.getElementById('iv-create-form').addEventListener('submit', handleCreate);
  document.getElementById('iv-back-btn').addEventListener('click', showListView);
  document.getElementById('iv-research-btn').addEventListener('click', runResearch);
